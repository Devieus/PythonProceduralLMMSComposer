...
"""
This only generates drums with LMMS' plug-ins.
Note that just about everything can be a drum, as long as the note length is set to -194 (for some reason).
...
def createInstrument(instrumentName,shapeSample,basenote):
    ...
    "vol3":"15",
    "crs3":"0",
    "on4":"1", #channel 4 is the noise channel, used for drums.
    "vol4":"15",
    ...
    "waveForm": "3", #waveform, square 0, saw 1, sine 2, noise 3. All other values are normals.
    "att": "0", #attack.
    "hold": "0", #hold (time until sustain).
    "sus": "0", #punch (very short).
    "dec": "0.1", #decay (sustain time).
    "startFreq": str(r.choice(range(600))/1000+.2), #0.352 is natural, use a range for drums.
    ...
    "waveform0":"3", # Tri wave (0), square wave (1), sawtooth (2), noise (3). Note that the noise is pitched.
    "ringmod0":"0", # Enables the ring mod for the tone generator.
    ...
    if instrumentName == "malletsstk": return {
        "preset":r.choice(["0", "1", "2", "5", "8"]),
