...
3: Calypso
4: Free jazz"""
songType=0#r.choice(range(5))
print(f"songtype: {songType}")
# Variables
outroPosition=0
# A bar is ticks long, if the numerator and denominator is 4
# and 144 when the signature is 3/4.
# If the denominator remains fixed, bar lengths would be numerator*48
try: numerator=int(input("Numerator: "))
except ValueError:
        numerator=r.randint(2,7) if songType!=3 else 4
steps=numerator*4
ticks=numerator*48
"""Drum track
Harmony
...
Basically the song duration will be taken into account (bpm*beats(=ticks/4)=1 maybe) to check on the upper bound.
...
sectionLengths={"intro":r.randint(1,3)*2,
    ...
    "bridge":r.randint(0,8),
    "outro":r.randint(0,8),
    }
# A place to store valuables.
# Notes go here, the key (pitch), relative position (just add bars*ticks for absolute) and length.
...
"""
So for the sake of argument, a minute song itself will be a total of ticks len/bar, 4 beats/bar, 120 beats/min
So 30 bars=30*ticks=5760 len per minute at 120bpm.
Alternatively it's bpm/numerator*ticks
...
bpm=r.randint(songbars,songbars*3)
...
for section in songDic:
    ...
    if sectionLengths[section]>0:
        ...
        bassPattern=ET.Element("pattern",{"pos":0,"type":"0","muted":"0","name":section,"steps":f"{steps}"})
        LMMSutil.makeBass(bassPattern,progDic[section],sectionLengths[section],harmDic[section],songType,ticks=ticks)
        # The pattern contains the notes.
        harmonyPattern=ET.Element("pattern", {"pos":0, "type": "0", "muted": "0", "name":section, "steps": "16"})
        # Every section has its own length (in bars).
        LMMSutil.makeHarmonies(harmonyPattern,section,progDic[section],numerator,sectionLengths[section],songType,ticks=ticks)
        # At this point,the lead needs representation as well.
        leadPattern=ET.Element("pattern",{"pos":0,"type":"0","muted":"0","name":section,"steps":f"{steps}"})
        # However, it is done with full melodic tracks.
        LMMSutil.makeMelody(leadPattern,progDic[section],sectionLengths[section],ticks)
        # The call track as the lead.
        callPattern=ET.Element("pattern",{"pos":0,"type":"0","muted":"0","name":section,"steps":f"{steps}"})
        LMMSutil.makeMelody2(callPattern, progDic[section], sectionLengths[section],True,ticks=ticks)
        # The response track to talk with the call.
        responsePattern=ET.Element("pattern",{"pos":0,"type":"0","muted":"0","name":section,"steps":f"{steps}"})
        LMMSutil.makeMelody2(responsePattern,progDic[section],sectionLengths[section],False,ticks=ticks)
        # Now to add these patterns to the songDic.
        songDic[section].append(bassPattern)
        songDic[section].append(harmonyPattern)
...
LMMSutil.makeInstrument(bassTrack,instrumentName=r.choice(["bitinvader","nes","papu","sid","tripleoscillator","watsyn"]),basenote="69",instrumentType=songType,vol="30")

bassTrack2=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":LMMSutil.word(2)})
LMMSutil.makeInstrument(bassTrack2,instrumentName=r.choice(["bitinvader","nes","papu","sid","tripleoscillator","watsyn"]),basenote="69",instrumentType=songType,vol="30")

bassTrackBridge=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":LMMSutil.word(2)})
LMMSutil.makeInstrument(bassTrackBridge,instrumentName=r.choice(["bitinvader","nes","papu","sid","tripleoscillator","watsyn"]),basenote="69",instrumentType=songType,vol="30")
...
LMMSutil.makeInstrument(harmonyTrack,instrumentName=instrumentName,basenote="45",vol="30",instrumentType=songType)

harmonyTrack2=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":LMMSutil.word(2)})
instrumentName="malletsstk" if songType==1 else r.choice(["bitinvader","monstro","nes","papu","sfxr","sid","tripleoscillator","watsyn"])
LMMSutil.makeInstrument(harmonyTrack2,instrumentName=instrumentName,basenote="45",vol="30",instrumentType=songType,env=1)

harmonyTrackBridge=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":LMMSutil.word(2)})
instrumentName="malletsstk" if songType==1 else r.choice(["bitinvader","monstro","nes","papu","sfxr","sid","tripleoscillator","watsyn"])
LMMSutil.makeInstrument(harmonyTrackBridge,instrumentName=instrumentName,basenote="45",vol="30",instrumentType=songType,env=1)

# Now for the sting. Only once per bar, max.
leadTrack=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":"lead"})
# Only meditative uses SF2, use a boolean to select it inline.
LMMSutil.makeInstrument(leadTrack,instrumentName=[r.choice(["bitinvader","monstro","nes","papu","sfxr","sid","tripleoscillator","watsyn"]),"sf2player"][songType==1],basenote="45",instrumentType=songType)
callTrack=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":"call"})
# It might not necessarily be only meditative, though genres that Also use SF2 are going to be an issue.
LMMSutil.makeInstrument(callTrack,instrumentName=[r.choice(["bitinvader","nes","papu","sfxr","sid","tripleoscillator","watsyn"]),"sf2player"][songType==1],basenote="45",instrumentType=songType)
# Call/response track that harmonizes during the verse.
responseTrack=ET.SubElement(trackcontainer,"track",{"type":"0","muted":"0","solo":"0","name":"resposne"})
instrumentName="sf2player" if songType==1 else r.choice(["bitinvader","nes","papu","sfxr","sid","tripleoscillator","watsyn"])
LMMSutil.makeInstrument(responseTrack,instrumentName=instrumentName,basenote="57", instrumentType=songType)
...
for section in structure:
    ...
    if sectionLengths[section]>0:
        if section=="intro":
            ...
            outro=ET.SubElement(bassTrack2,"pattern",{"pos":str(position),"steps":f"{steps}","type":"1","muted":"0","name":"outro"})
            ...
            # And this harmony
            outro=ET.SubElement(harmonyTrack2,"pattern",{"pos":str(position),"steps":f"{steps}","type":"1","muted":"0","name":"outro"})
...
#--------------------Stings-----------------------------------
# #birdPattern=ET.SubElement(birdTrack,"pattern",{"pos":"0","type":"0","muted":"0","name":LMMSutil.word(2),"steps":f"{steps}"})
pos=0
# The bird will play a fixed melody at random points.
# This melody will be a half-bar long, which means its total length is 96
birdMelody=ET.Element("pattern",{"pos":"0","type":"0","muted":"0","name":LMMSutil.word(2),"steps":f"{steps}"})
for x in range(2):
...
birdMelody2=ET.Element("pattern",{"pos":"0","type":"0","muted":"0","name":LMMSutil.word(2),"steps":f"{steps}"})
for x in range(2):
    # Melodies are four bars long (reduced to one bar due to the low BPM), it starts with a half note, followed by 4 quarter notes.
...
# Someone wanted a dinosaur. This is the dinosaur.
dinosaurMelody=ET.Element("pattern",{"pos":"0","type":"0","muted":"0","name":LMMSutil.word(2),"steps":f"{steps}"})
...
for _ in range(20):
    # Melodies are two bar long, notes placed should have their pos limited to ticks. Since the note lengths are still a thing, it might overshoot. That's fine.
...
for x in range(3):
    ...
    pattern=ET.SubElement(drumtrack,"pattern",{"pos":"0","type":"0","muted":"0","name":"","steps":str(steps*2)})
    if songType==3:LMMSutil.makeDrumpattern(pattern, 25, songType + x, steps)
    else: LMMSutil.makeDrumpattern(pattern, 25, songType, steps)
    # So here's how multiple drum tracks happen:
    pattern = ET.SubElement(drumtrack,"pattern",{"pos":"0","type":"0","muted":"0","name":"","steps":str(steps*2)})
    # You just kinda make another pattern.
    if songType==3:LMMSutil.makeDrumpattern(pattern, 50, songType + x, steps)
    else:LMMSutil.makeDrumpattern(pattern, 50, songType + x, steps)
...
ET.SubElement(automationTrack,"automationpattern",{"tens":"1",
                                                   ...
                                                   "pos":"0",
                                                   "len":f"{ticks}"})
# That's because ticks is divisible by 1,2,3,4,6,8,12,and any other power of 2 multiplied by 3.
ET.SubElement(automationTrack,"automationpattern",{"tens":"1",
                                                   "mute":"0",
                                                   "name":"Denominator",
                                                   "pos":"0",
...
key:{['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'][LMMSutil.key]}"
# Set the loop points.
